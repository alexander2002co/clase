<?php
	...
	if (!empty($_POST['ingresar'])) {
		if (empty($_POST['maquinaria'])||empty($_POST['silo'])||empty($_POST['fecha'])||empty($_POST['hora'])||empty($_POST['hora_in'])||empty($_POST['hora_fin'])) {
		    ...
		    $INGRESAR="INSERT INTO `maquinaria`(`id_maquinaria`, `tipo`, `fecha`, `tiempo`, `tiempo_start`, `tiempo_end`, `inspector`)VALUES (".$i.",'".$_POST['maquinaria']."','".$_POST['fecha']."','".$_POST['hora']."','".$_POST['hora_in']."','".$_POST['hora_fin']."',".$_SESSION['id_usuario'].")";
		    if ($mysqli->query($INGRESAR)){
		    	$update="UPDATE `descargue` SET `maquinaria`=".$i." WHERE `id_ingre`=".$_POST['silo'];
...
<body>
	<div id="ext" class="container menus w-50 p-3">
		...
		<h1 class="lead separa" align="center">Maquinaria</h1>
		<form method="post">
		...
		<div class="input-group mb-3">
		  <input type="time"  class="form-control" placeholder="Recipient's username" name="hora_fin" aria-label="Recipient's username" aria-describedby="basic-addon2">
		  <div class="input-group-append">
		...
		<div class="input-group mb-3 ">
		  <select class="custom-select" id="inputGroupSelect02" name="silo">
			<option selected value="">Selecciona un campo</option>
			<?php
				$consulta="SELECT * from descargue order by id_ingre asc";
				$resultado=$mysqli->query($consulta);
				$TotReg=mysqli_num_rows($resultado);
				foreach ($resultado as $reg) {
					...
					echo '<option value="'.$reg['id_ingre'].'">ID: '.$reg['id_ingre'].' - '.$datos[0].'</option>';
				}
			?>
		  </select>
		  <div class="input-group-append separa">
